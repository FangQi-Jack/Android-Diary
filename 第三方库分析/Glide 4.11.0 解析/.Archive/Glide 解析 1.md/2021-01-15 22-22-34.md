...
```
在 load 方法中首先调用了 `asDrawable` 方法，该方法中又调用了 as 方法并床 Drawable.class ，
```
/**
...
**总结：load 方法主要用于创建一个 RequestBuilder。**
...
```
在 into 方法中对当前是否 UI 线程做了判断，因为只有 UI 线程才能更新 UI，即为 ImageView 设置图片。接着条用了 into 方法，并在参数中调用了 `glideContext.buildImageViewTarget` 方法。
```
@NonNull
...
接着看 begin 方法，如果功能 override 方法指定了宽高，则调用 `onSizeReady` 方法，否则调用 Target 的 getSize 方法，该方法通过 target 获取到宽高后也会调用 `onSizeReady` 方法：
```
...
```
首先会进入 INITIALIZE，在这里调用了 runGenerators：
```
...
```
通过 HttpUrlConnection 完成图片网络请求。完成请求后回调 callback 的 onDataReady，进而回调 onDataReadyInernal：
```
@Synthetic
...
```
如果没有配置缓存则执行 cb.onDataFetcherReady 方法，这里的 cb 是 FetcherReadyCallback，DecodeJob 实现了
