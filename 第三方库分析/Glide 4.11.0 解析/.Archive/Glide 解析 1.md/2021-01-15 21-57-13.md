...
```
Engine 的 load 方法中首先通过参数构建了 key，它是一个 EngineKey，用作内存缓存的 key，然后尝试通过 key 从内存缓存中读取缓存，如果读取到了，则直接用这份缓存数据调用 SingleRequest 的 `onResourceReady` 方法，则会回到上文中的状态为 COMPLETE 的流程。否则调用
