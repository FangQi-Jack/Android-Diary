...
## 磁盘缓存的写入时机
如上文分析，在 DecodeJob 的 `runGenerators()` 方法中，如果 DataCacheGenerator 无法获取到磁盘缓存，将继续执行 `getNextStage` 和 `getNextGenerator` 方法，此时 stage 将变为 Stage.SOURCE，currentGenerator 将变为 SourceGenerator，如di
